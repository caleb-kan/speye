  ...
  if (!paused) {
    ...
    if (webgazer.params.showFaceOverlay) {
      ...
      tracker.drawFaceOverlay(
        faceOverlay.getContext('2d', { willReadFrequently: true }),
...
async function init(stream) {
  ...
  // Only add mouse listeners if useMouseData is enabled
  // When disabled, gaze prediction relies only on explicit calibration
  if (webgazer.params.useMouseData) {
...
webgazer.begin = async function (onFail) {
  ...
  curTracker = new webgazer.tracker.TFFaceMesh()
...
webgazer.recordScreenPosition = function (x, y, eventType) {
  ...
  // Save to localforage so calibration data persists across sessions
  if (webgazer.params.saveDataAcrossSessions) {
    setGlobalData()
  }
